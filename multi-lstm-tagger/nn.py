...
class EmbeddingLayer(object):
    ...
    def __init__(self, input_dim, output_dim, name='embedding_layer'):
        ...
        print "EmbeddingLayer (nn.py)", "name:", name, "; input dim:", input_dim, "; output_dim:", output_dim
...
def forward1(observations, transitions, observations1, transitions1, viterbi=False,
            return_alpha=False, return_best_sequence=False):
    """
    Takes as input:
        - observations, sequence of shape (n_steps, n_classes)
    ...
    initial1 = observations1[0]
    alpha1, _ = theano.scan(
        fn=recurrence,
        outputs_info=(initial1, None) if return_best_sequence else initial1,
        sequences=[observations1[1:]],
        non_sequences=transitions1
    )
    ...
    if return_alpha:
        ...
        sequence1, _ = theano.scan(
            fn=lambda beta_i, previous: beta_i[previous],
            outputs_info=T.cast(T.argmax(alpha1[0][-1]), 'int32'),
            sequences=T.cast(alpha1[1][::-1], 'int32')
        )
        sequence1 = T.concatenate([sequence1[::-1], [T.argmax(alpha1[0][-1])]])
        ...
        if viterbi:
            return [alpha[-1].max(axis=0), alpha1[-1].max(axis=0)]
        else:
            return [log_sum_exp(alpha[-1], axis=0), log_sum_exp(alpha1[-1], axis=0)]




def forward_n(arr, viterbi=False,
            return_alpha=False, return_best_sequence=False):
    """
    Takes as input:
        - observations, sequence of shape (n_steps, n_classes)
    ...
    assert not return_best_sequence or (viterbi and not return_alpha)
    print "nn.forward_n()"
    # observations, transitions, observations1, transitions1 = arr
    ...
    alpha_arr = []
    initial_arr = []
    for observations, transitions in arr:
        initial = observations[0]
        alpha, _ = theano.scan(
            fn=recurrence,
            outputs_info=(initial, None) if return_best_sequence else initial,
    ...
    if return_alpha:
        ...
        if viterbi:
            return [alpha[-1].max(axis=0) for alpha in alpha_arr]
        else:
            return [log_sum_exp(alpha[-1], axis=0) for alpha in alpha_arr]
