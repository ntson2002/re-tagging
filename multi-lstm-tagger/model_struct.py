...
class Model(object):
        ...
        for name, param in self.components.items():
            if hasattr(param, 'params'):
                for p in param.params:
                    arr.append([p.name, p.get_value().shape, mul(p.get_value().shape)])
            else:
                arr.append([name, param.get_value().shape, mul(param.get_value().shape)])

        total = 0
        arr1 = []
        for r in arr:
            if "embeddings" in r[0]:
                arr1.append(r)
                total = total + r[2]
        arr1.append(["total embeddings:", None, total])


        total = 0
        arr2 = []
        for r in arr:
            if not "embeddings" in r[0]:
                arr2.append(r)
                total = total + r[2]
        arr2.append(["total parameters:", None, total])
        return arr1, arr2
    ...
    def build(self,
        ...
        assert model_type in {"struct", "struct_mlp", "struct_mlp2", "multilayer", "single"}
        ...
        if model_type == "multilayer" or model_type == "single":
            ...
            for ilayer in range(len(self.tag_maps)):
                inputs_i = previous_inputs if prev_tags_cores == None else T.concatenate([previous_inputs, prev_tags_cores], axis=1)
                ...
                tag_ids_i = T.ivector(name='tag_ids' + str(ilayer))  # input tags of layer i
            ...
            if model_type == "struct":
                for ilayer in range(0, len(self.tag_maps)):
                    n_tags_i = len(self.tag_maps[ilayer]['id_to_tag'])
                    final_output = final_output if prev_tags_cores == None else T.concatenate( [final_output, prev_tags_cores], axis=1 )
                    final_layer_i = HiddenLayer(word_lstm_dim + prev_ntags, n_tags_i, name='final_layer_' + str(ilayer),
                                                activation=(None if crf else 'softmax'))
                ...
                for ilayer in range(0, len(self.tag_maps)):
                    ...
                    if model_type == "struct_mlp2":
                        mlp_sizes = [word_lstm_dim + prev_ntags, word_lstm_dim, word_lstm_dim]
                    else:
                        mlp_sizes = [word_lstm_dim + prev_ntags, word_lstm_dim]

                    mlp_input = final_output
                    for j in range(len(mlp_sizes) - 1):
                        mlp_layer =  HiddenLayer(mlp_sizes[j], mlp_sizes[j + 1],
                                               name="mlp" + str(j+1) + "_layer_" + str(ilayer), activation="tanh")
