...
def get_path(parameters):
    ...
    """
    l = []
    # selected_keys = {'model_type', 'tag_scheme', 'word_dim', 'word_bidirect', 'pre_emb', 'crf', 'all_emb', 'external_features', 'dropout', 'char_dim', 'prefix'}
    selected_keys = {'model_type', 'prefix'}
    for k, v in parameters.items():
...
def evaluate_multilayer(parameters, f_eval, raw_sentences, parsed_sentences, tag_maps,
    ...
    for raw_sentence, data in zip(raw_sentences, parsed_sentences):
        ...
        gold_colums = [x["column"] for x in tag_maps]
        for tm in tag_maps:
            ...
            for i, (y_pred, y_real) in enumerate(zip(y_preds, y_reals)):
                ...
                new_line = " ".join(raw_sentence[i][:1] + [raw_sentence[i][x] for x in gold_colums] + [r_tags[i], p_tags[i]])
                # print new_line
                # print "========="
    ...
    for tm in tag_maps:
        ...
        output_path = os.path.join(eval_temp, "eval.%i.layer%s.output" % (eval_id, tm['layer']))

        scores_path = os.path.join(eval_temp, "eval.%i.layer%s.scores" % (eval_id, tm['layer']))
...
def parse_feature_string(feature_string):
    ...
    if feature_string != "None":
        features = [{'name': y[0], 'column': int(y[1]), 'dim': int(y[2])}
                    for y in [x.split('.') for x in feature_string.split(",")]]
    return features
...
def print_evaluation_result(results, print_detail=True, f = None):
    sentences = []
    ...
    if print_detail:
        for item in results:
            layer = item['_layer']
            sentences.append("* Conlleval result of LAYER %s:" %layer)
            sentences.append(item['conlleval'])

    sentences.append("")
    sentences.append("LAYER\tTOTAL\tFOUND\tCORRECT\tPrecision\tRecall\tF1")

    total_total_phrased = 0
    total_total_found = 0
    total_correct = 0
    ...
    for item in results:
        layer = item['_layer']
        # sentences.append("")
        # sentences.append("*** LAYER: %s" % layer)
        # sentences.append("- Precision: %.5f Recall: %.5f FB1: %.5f" %(item['result']['precision'], item['result']['recall'], item['result']['FB1']))
        # sentences.append("- Gold: %d ; Found: %d ; Correct: %d" % (item['result']['total_phrased'], item['result']['total_found'], item['result']['correct']))
        # if printcm == True:
        #     sentences.append("* Confusion matrix:")
        #     sentences.append(item['confusion_table'])
        # sentences.append("* Conlleval result: ")
        # sentences.append(item['conlleval'])
        # sentences.append("")
        r = item["result"]
        total_total_phrased += r["total_phrased"]
        total_total_found += r["total_found"]
        total_correct += r["correct"]
        sentences.append("LAYER %d\t%d\t%d\t%d\t%f\t%f\t%f" % (layer, r["total_phrased"], r["total_found"],
                                                               r["correct"], r["precision"], r["recall"], r["FB1"]))

    precision = 0 if total_total_found == 0 else total_correct / (1.0 * total_total_found)
    recall = 0 if total_total_phrased == 0 else total_correct / (1.0 * total_total_phrased)
    fb1 = 0 if precision + recall == 0 else (2 * precision * recall) / (precision + recall)

    sentences.append("OVERALL\t%d\t%d\t%d\t%f\t%f\t%f\n" % (total_total_phrased, total_total_found,
                                                          total_correct, precision * 100, recall * 100, fb1 * 100))
